...
export class CreateReceiptVoucherDto {
    ...
    @ApiProperty({ description: 'Source type', example: VoucherSourceType.PARTNER, enum: VoucherSourceType })
    @IsNotEmpty()
    @IsEnum(VoucherSourceType)
    source_type: VoucherSourceType;

    @ApiProperty({ description: 'Name (if source is Others)', example: 'John Smith' })
    @IsOptional()
    @IsString()
    name?: string;
    ...
    @ApiProperty({ description: 'Check ID (if payment method is Check)', example: 1 })
    @IsOptional()
    @IsNumber()
    check_id?: number;
}
