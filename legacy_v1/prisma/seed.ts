...
async function seed() {
    ...
    try {
        ...
        // Note: The original seed has Partner model which is commented out in schema but used here? 
        // Wait, Partner model IS in the schema I copied just now.
        // But the seed references Partner. Let's assume it works as written.
        // Wait, the seed has create: { ...partner_code... } but the Partner model in schema might have 'partner_code' as unique.

        // Error handling for Partner if model issues exist.
        // In the copied schema, model Partner exists.
        const partners = await Promise.all([
            /* 
               We need to be careful. The Partner model in schema has:
               model Partner {
                 id              Int      @id @default(autoincrement())
                 partner_code    String   @unique
                 ...
               }
               The seed uses `partner_code` in where/create. This matches.
            */
        ...
        // We need to handle related records carefully. 
        // Quotation needs project_type, customer.
        // The seed creates them first so it should be fine.

        // Fix: Quotation model uses `items` relation via `QuotationItem` model.
        // The seed uses `items: { create: [...] }` which is correct for nested writes.
        ...
        await prisma.revenue.createMany({
            ...
            skipDuplicates: true // Good practice for repeated seeds
        });
        ...
        // ReceiptVoucher
        const existingReceipt = await prisma.receiptVoucher.findUnique({ where: { voucher_number: 'RV-2025-001' } });
        if (!existingReceipt) {
        ...
        const existingPayment = await prisma.paymentVoucher.findUnique({ where: { voucher_number: 'PV-2025-001' } });
        if (!existingPayment) {
